...
drawer.innerHTML = `
  ...
  <div class="rag__status" id="ragStatus" data-state="idle">
    <span class="rag__status-dot"></span><span id="ragStatusText">WAKE ON FIRST QUESTION</span>
  </div>
...
companion.innerHTML = `
  ...
  <span class="anis-bot" aria-hidden="true">
    ...
    <i class="anis-bot__arm anis-bot__arm--l"></i><i class="anis-bot__arm anis-bot__arm--r"></i>
    <i class="anis-bot__leg anis-bot__leg--l"></i><i class="anis-bot__leg anis-bot__leg--r"></i>
  </span>`;
...
// Wait until the other portfolio modules have appended Treasure Hunter, then watch the true final project.
setTimeout(()=>{
  ...
  if(lastProject&&'IntersectionObserver' in window){
    const observer=new IntersectionObserver(entries=>{
      ...
      if(entries.some(entry=>entry.isIntersecting&&entry.intersectionRatio>.22)){
        announced=true;companion.classList.add('show');observer.disconnect();
      }
    },{threshold:[.22,.45]});
    ...
    window.addEventListener('scroll',()=>{
      ...
      if(r.bottom<window.innerHeight*1.25&&r.bottom>0){announced=true;companion.classList.add('show')}
    },{passive:true});
  }
},0);
...
if(nav&&terminalButton&&!nav.querySelector('[data-ask-anis]')){
  const ask=document.createElement('button');ask.dataset.askAnis='true';ask.textContent='ASK ANIS';ask.addEventListener('click',openDrawer);nav.insertBefore(ask,terminalButton);
}

const client=document.createElement('script');client.src='rag.js';client.defer=true;document.body.appendChild(client);
